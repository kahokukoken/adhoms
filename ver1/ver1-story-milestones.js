(() => {
  ...
  function pendingDirective() {
    for (const number of [1, 2, 3]) {
      if (window.ADHOMS_LIGHT_STATE.flags?.[`directive:${number}:seen`] &&
          !window.ADHOMS_LIGHT_STATE.flags?.[`directive:${number}:ack`]) return number;
    }
    return null;
  }

  function showDirective(number) {
    if (!DIRECTIVES[number] || window.ADHOMS_LIGHT_STATE.flags?.[`directive:${number}:ack`]) return false;
    const host = overlay();
  ...
  function queueDirective(number) {
    if (!DIRECTIVES[number] || window.ADHOMS_LIGHT_STATE.flags?.[`directive:${number}:ack`]) return;
    window.ADHOMS_LIGHT_STATE.flags[`directive:${number}:seen`] = true;
    save();
    tryResumePending();
  }
  ...
  function tryResumePending() {
    const host = overlay();
    if (!host || isOpen(host)) return;
    const number = pendingDirective();
    if (number) showDirective(number);
  }
  ...
  window.nextMonth = function nextMonthWithStoryMilestones() {
    ...
    if (before < 48 && after >= 48 && !window.ADHOMS_LIGHT_STATE.flags?.['directive:3:ack']) {

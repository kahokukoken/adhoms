...
ALIASES={
    "manutd":"manunited","manchesterunited":"manunited","manchestercity":"mancity",
    "nottinghamforest":"nottmforest","newcastleunited":"newcastle",
    "tottenhamhotspur":"tottenham","spurs":"tottenham",
    "wolverhamptonwanderers":"wolves","brightonandhovealbion":"brighton",
    "westhamunited":"westham","leicestercity":"leicester","ipswichtown":"ipswich",
    "sheffieldutd":"sheffieldunited","lutontown":"luton","leedsunited":"leeds"
}
...
def get_csv(url):
    r=requests.get(url,timeout=60,headers={"User-Agent":"Mozilla/5.0 ADHOMS-Football-Research/1.3"}); r.raise_for_status()
    return pd.read_csv(io.StringIO(r.content.decode("utf-8-sig",errors="replace")),on_bad_lines="skip")

def norm3(a,b,c):
    q=1/np.array([float(a),float(b),float(c)],dtype=float); return q/q.sum()
...
def build_season(season):
    ...
    fd=fd.dropna(subset=need)
    market={}
    for _,r in fd.iterrows():
        market[(str(r.Date.date()),keyname(r.HomeTeam),keyname(r.AwayTeam))]=(
            r.FTR,norm3(r.AvgH,r.AvgD,r.AvgA),norm3(r.AvgCH,r.AvgCD,r.AvgCA))
    ...
    for fixture,g in gw.groupby("fixture",sort=False):
        ...
        result,opening,closing=market[mk]
        ...
        rows.append({
            ...
            "oH":opening[0],"oD":opening[1],"oA":opening[2],"cH":closing[0],"cD":closing[1],"cA":closing[2],
            "close_lr_h":math.log(closing[0]/closing[2]),"close_lr_d":math.log(closing[1]/closing[2]),
            "lineup_attack_diff":h["attack"]-a["attack"],"individual_peak_diff":h["peak"]-a["peak"],
            "disruption_diff":h["disruption"]-a["disruption"],"continuity_diff":h["continuity"]-a["continuity"],
            "lineup_threat_diff":h["threat"]-a["threat"],"home_known_starters":h["n"],"away_known_starters":a["n"]
        })
...
def main():
    ...
    market=["close_lr_h","close_lr_d"]
    extras=["lineup_attack_diff","individual_peak_diff","disruption_diff","continuity_diff","lineup_threat_diff"]
    variants={"closing_calibrated":market,**{e:market+[e] for e in extras},"all_lineup_features":market+extras}
    reports=[]; windows={k:[] for k in variants}
    for i,s in enumerate(SEASONS[1:],start=1):
        train=rows[rows.season.isin(SEASONS[:i])]; test=rows[rows.season==s]; y=test.y.to_numpy(int)
        O=test[["oH","oD","oA"]].to_numpy(float); C=test[["cH","cD","cA"]].to_numpy(float)
        ob,ol=brier(O,y),logloss(O,y); cb,cl=brier(C,y),logloss(C,y)
        rep={"season":s,"n":int(len(test)),"opening_market_brier":ob,"opening_market_logloss":ol,"closing_market_brier":cb,"closing_market_logloss":cl,"variants":{}}
        for name,fs in variants.items():
            P=fit_predict(train,test,fs); vb,vl=brier(P,y),logloss(P,y)
            x={"brier":vb,"logloss":vl,"brier_delta_vs_closing":vb-cb,"logloss_delta_vs_closing":vl-cl}
            rep["variants"][name]=x; windows[name].append(x)
        reports.append(rep)
    acceptance={}
    for name,ws in windows.items():
        wb=sum(x["brier_delta_vs_closing"]<0 for x in ws); wl=sum(x["logloss_delta_vs_closing"]<0 for x in ws)
        mb=float(np.mean([x["brier_delta_vs_closing"] for x in ws])); ml=float(np.mean([x["logloss_delta_vs_closing"] for x in ws]))
        acceptance[name]={"windows_brier_better_than_closing":wb,"windows_logloss_better_than_closing":wl,"mean_brier_delta":mb,"mean_logloss_delta":ml,"candidate":bool(wb==len(ws) and wl==len(ws) and mb<0 and ml<0)}
    summary={
        "mode":"post-lineup pre-kickoff research backtest",
        "hard_benchmark":"normalized Football-Data market-average closing 1X2 probabilities",
        "timing_rule":"current fixture contributes only starts; all player ability statistics come strictly from prior fixtures",
        "train":"2022-23","evaluation":["2023-24","2024-25"],
        "matched_rows_by_season":{s:int(len(f)) for s,f in zip(SEASONS,frames)},
        "unmatched_fixtures_by_season":{s:int(f.attrs.get('unmatched_fixtures',0)) for s,f in zip(SEASONS,frames)},
        "features":extras,"seasons":reports,"acceptance":acceptance,
        "candidate_rule":"Candidate requires beating closing market on both Brier and log loss in both evaluation seasons; candidate is still not evidence of executable profit."
    }
